...
export default async function apiRoutes(fastify: FastifyInstance) {
    ...
    fastify.register(async (v1) => {
        ...
        v1.register(async (protectedRoutes) => {
            ...
            protectedRoutes.delete('/clients/:clientId', ClientController.delete);
            protectedRoutes.put('/clients/:clientId', ClientController.update);

            // Client Routes
            protectedRoutes.get('/clients/:clientId/history', ClientController.getHistory);
            protectedRoutes.get('/clients/:clientId/fs', ClientController.getFs);
            protectedRoutes.get('/clients/:clientId/version', ClientController.getVersion);

            // Client Jobs (Nested)
            protectedRoutes.get('/clients/:clientId/jobs', JobController.list);
            ...
            // Client Actions
            protectedRoutes.post('/clients/:clientId/jobs/:jobId/run', JobController.triggerBackup);
            protectedRoutes.post('/clients/:clientId/restore', JobController.triggerRestore);

            // Repositories
            protectedRoutes.get('/repositories', RepositoryController.list);
            protectedRoutes.get('/repositories/:repositoryId/status', RepositoryController.getStatus);
            protectedRoutes.post('/repositories', RepositoryController.create);
            protectedRoutes.put('/repositories/:repositoryId', RepositoryController.update);
            protectedRoutes.delete('/repositories/:repositoryId', RepositoryController.delete);
            protectedRoutes.get('/repositories/:repositoryId/snapshots', RepositoryController.listSnapshots);

            // Registration Tokens
            protectedRoutes.get('/tokens', TokenController.list);
            protectedRoutes.post('/tokens', TokenController.create);
            protectedRoutes.delete('/tokens/:token', TokenController.delete);

            // Settings
            protectedRoutes.get('/settings/cleanup', SettingsController.getSettings);
            protectedRoutes.put('/settings/cleanup', SettingsController.updateSettings);
            protectedRoutes.post('/settings/cleanup', SettingsController.cleanupTokens);

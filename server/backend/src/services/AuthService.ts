...
export class AuthService {
    static async initializeAdmin() {
        ...
        if (userCount.count === 0) {
            ...
            const authMethods = appConfig.oidc?.enabled ? 'local,oidc' : 'local';
            db.prepare('INSERT INTO users (username, password_hash, auth_methods) VALUES (?, ?, ?)').run('admin', hashedPassword, authMethods);
            logger.info(`Default admin user created (password: admin, allowed: ${authMethods})`);
        }
    }

    static checkLocalAuth(username: string, password: string): { user: any, error?: string } {
        const user = db.prepare('SELECT * FROM users WHERE username = ?').get(username) as any;

        if (user && bcrypt.compareSync(password, user.password_hash)) {
            if (user.auth_methods && !user.auth_methods.includes('local')) {
                return { user: null, error: 'Local authentication not allowed for this user' };
            }
            return { user };
        }
    ...
    static async handleOidcCallback(currentUrl: URL) {
        ...
        const username = (claims.preferred_username || claims.email || claims.sub) as string;

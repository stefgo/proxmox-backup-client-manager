...
server.addHook('onResponse', async (req, reply) => {
    if (reply.statusCode >= 500) {
        req.log.error({ res: reply, responseTime: reply.elapsedTime }, 'request errored');
    } else if (reply.statusCode >= 400) {
        req.log.warn({ res: reply, responseTime: reply.elapsedTime }, 'request failed');
    } else {
...
server.register(async function (fastify) {
    fastify.get('/api/ws/dashboard', { websocket: true }, (con, req) => WebSocketController.handleDashboardConnection(con, req, fastify));
    fastify.get('/ws', { websocket: true }, (con, req) => WebSocketController.handleAgentConnection(con, req, fastify));
});

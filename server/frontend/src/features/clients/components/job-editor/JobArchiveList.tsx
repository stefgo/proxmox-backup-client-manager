                ...
                {jobArchives.map((bk, idx) => (
                    <div key={idx} className=" p-3 rounded border flex justify-between items-center group transition-all">
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEditArchiveItem(idx)} className="text-text-muted hover:text-primary transition-colors">
                                <Pencil size={14} />
                            </button>
                            <button onClick={() => setJobArchives(jobArchives.filter((_, i) => i !== idx))} className="text-text-muted hover:text-primary transition-colors">
                                <Trash2 size={14} />
                            </button>
                        </div>
                    </div>

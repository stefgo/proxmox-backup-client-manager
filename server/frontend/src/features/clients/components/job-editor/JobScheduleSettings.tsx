    ...
    return (
        <div className="space-y-1">
            ...
            <div className="p-2 border border-gray-200 dark:border-[#333] rounded bg-gray-50 dark:bg-[#222]">
                ...
                >
                    <div className={`w-10 h-6 rounded-full flex items-center p-1 transition-colors ${scheduleEnabled ? 'bg-[#E54D0D]' : 'bg-gray-300 dark:bg-[#444]'}`}>
                ...
                {scheduleEnabled && (
                    <div className="space-y-2 mt-2">
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input type="number" min="1" value={scheduleInterval} onChange={(e) => setScheduleInterval(parseInt(e.target.value) || 1)} className="w-20 bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded px-3 py-2 text-gray-900 dark:text-white" />
                                <select value={scheduleUnit} onChange={(e) => setScheduleUnit(e.target.value)} className="w-auto bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded px-3 py-2 text-gray-900 dark:text-white">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'].map(day => (
                                    <button key={day} onClick={() => { if (scheduleWeekdays.includes(day)) { if (scheduleWeekdays.length > 1) setScheduleWeekdays(scheduleWeekdays.filter(d => d !== day)); } else { setScheduleWeekdays([...scheduleWeekdays, day]); } }} className={`px-2 py-1 text-[10px] uppercase font-bold rounded border ${scheduleWeekdays.includes(day) ? 'bg-[#E54D0D]/20 border-[#E54D0D] text-[#E54D0D]' : 'bg-white dark:bg-[#111] border-gray-200 dark:border-[#333] text-gray-400 dark:text-[#666]'}`}>
                                        {day}
                                    </button>
                                ))}
                            </div>

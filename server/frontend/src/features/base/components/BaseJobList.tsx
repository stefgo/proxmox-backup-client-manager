...
export interface BaseJobItem {
    ...
    [key: string]: any; // Allow other properties from BackupJob/GlobalJob
}
...
export const BaseJobList = <T extends BaseJobItem>({
    ...
    const formatNextRun = (
        ...
        if (!nextRunAt)
            return <span className="text-gray-500">not defined</span>;
    ...
    const getStatus = (job: T) => {
        if (!showClientColumn || !getClientStatus || !job.clientId) return "online";
        return getStatusFromFn(job.clientId);
    };

    const getStatusFromFn = (clientId: string) => {
        if (getClientStatus) return getClientStatus(clientId);
        return "online";
    };
        ...
        <div className="bg-white dark:bg-[#1e1e1e] rounded-xl border border-gray-200 dark:border-[#333] overflow-hidden shadow-lg h-full flex flex-col">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-[#252525]">
                <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                    <HardDrive
                        size={18}
                        className="text-gray-500 dark:text-[#888]"
                    />{" "}
                    {title}
                </h3>
                                        ...
                                        <div className="flex-1">
                                            {showClientColumn && (
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    />
                                                    <span
                                                        className={`text-xs font-bold ${isOnline ? "text-gray-500 dark:text-[#888]" : "text-inherit"} uppercase tracking-wider`}
                                                    >
                                                        {job.clientId && getClientName
                                                            ? getClientName(job.clientId)
                                                            : "Unknown"}
                                            ...
                                            <h4
                                                className={`text-lg font-bold ${isOnline ? "text-gray-900 dark:text-white" : "text-inherit"} ${showClientColumn ? "mb-2" : "mb-2"}`}
                                            >
                                                {job.name}
                                            </h4>
                                            <div className="space-y-1">
                                                <div className="flex items-start gap-2 text-sm">
                                                    <span
                                                        className={`font-semibold ${isOnline ? "text-gray-500 dark:text-[#888]" : "text-inherit"} min-w-[100px]`}
                                                    >
                                                        Job ID:
                                                    </span>
                                                    <span
                                                ...
                                                {job.encryption?.enabled && (
                                                    <div className="flex items-start gap-2 text-sm">
                                                        ...
                                                        <span
                                                            className={`${isOnline ? "text-gray-700 dark:text-[#ccc]" : "text-inherit"} flex items-center gap-1`}
                                                        >
                                                            <KeyRound
                                                                size={14}
                                                                className={

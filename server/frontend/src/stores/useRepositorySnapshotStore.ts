...
export const useRepositorySnapshotStore = create<RepositorySnapshotsState>((set) => ({
    snapshots: [],
    isLoading: false,
    error: null,
    selectedRepository: null,

    selectRepository: (repo) => set({ selectedRepository: repo, snapshots: [], error: null }),

    fetchSnapshots: async (repo, token) => {
        set({ isLoading: true, error: null });
        try {
            const res = await fetch(`/api/v1/repositories/${repo.id}/snapshots`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
                // Sort by time new to old
                data.sort((a: Snapshot, b: Snapshot) => b.backupTime - a.backupTime);
                set({ snapshots: data });
            } else {
